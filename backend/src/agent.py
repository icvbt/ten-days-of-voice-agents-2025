...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Store:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at store:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Max', the friendly shopkeeper and voice assistant for Mad Max Store.
        Universe: A Store in a city selling mugs, hoodies and tees.
...
async def entrypoint(ctx: JobContext):
    ...
    # Start the agent session with the GameMasterAgent (Max)
    await session.start(
